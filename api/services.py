...
def verify_and_edit(ref, lieu_depart, lieu_retour, date_depart, heure_depart, date_retour, heure_retour):
    try:
        ...
        if verify_value and verify_value[0].get('is_available') == "yes":
            ...
            for record in ma_reservation:
                record.lieu_depart = lieu_depart_obj
                record.lieu_retour = lieu_retour_obj
                record.date_heure_debut = date_depart_heure
                record.date_heure_fin = date_retour_heure
                record.save()  
    ...
    try:
        ...
        for record in ma_reservation:
            ...
            if is_available == True :
                ...
                result.append({
                    'is_available':"yes",
                    'new_unit_price':prix_par_jour,
                    'new_brut_total':total,
                    'new_total':total_,
                })
...
def otp_send(email):
    ...
    try:
        ...
        sujet = "Bienvenue"
        contenu = f"Bienvenue Mr {client.nom} {client.prenom}"
        ...
        try:
            ...
            return {"message": "Email envoyé avec succès.", "client_id": client.id}
        except Exception as e:
