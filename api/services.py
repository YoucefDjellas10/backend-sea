        ...
        if client_id :
            ...
            if client_verify:
                if client_verify.risque == "eleve":
                    ...
                    if phone and client_verify and client_verify.mobile != phone:
                        client_verify.mobile = phone
                        client_verify.save()
                    if email and client_verify and client_verify.email != email:
                        client_verify.email = email
                        client_verify.save()
...
def verify_and_calculate(ref, lieu_depart, lieu_retour, date_depart, heure_depart, date_retour, heure_retour, country_code):
    try:
        ...
        for record in ma_reservation:
            ...
            remaining_to_pay = round(new_total - float(record.montant_paye), 2) if country_code != "DZ" else round(new_total - (float(record.montant_paye) * taux_change), 2)
            ...
            result.append({
                ...
                'new_total_days': total_days,
                "new_klm_limit": new_klm_limit,
                'diff_days': total_days - record.nbr_jour_reservation,
                "diff_klm_limit": new_klm_limit - current_klm_limit,
                'old_total': old_total,
                'new_total': new_total,
                'frais': frais,
                'amount_paid': round(record.montant_paye, 2) if country_code != "DZ" else round(record.montant_paye, 2) * taux_change,
        ...
        if country_code =="DZ":
            ...
            if ma_reservation :
                result.append({
                    ...
                    "montant_paye":ma_reservation.montant_paye  * taux_change if ma_reservation.montant_paye else 0,
                    ...
                    "promotion_name": "Promotion" if ma_reservation.reduction and ma_reservation.reduction > 7 else None,
                    "promotion_name_ar": "تخفيض" if ma_reservation.reduction and ma_reservation.reduction > 7 else None,
                    "promotion_name_en": "Promotion" if ma_reservation.reduction and ma_reservation.reduction > 7 else None,
...
def otp_verify(email, otp, client_id):
    try:
        ...
        elif (str(client.otp) == str(otp) and timezone.now() - otp_time > timedelta(minutes=65)):
