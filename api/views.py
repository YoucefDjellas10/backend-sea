...
def confirmation_download(request):
    ...
    context = {
        ...
        "NOM_VEHICULE": livraison.modele.name,
        "MATRICULE": livraison.vehicule.matricule,
        "CAUTION": int(livraison.opt_protection_caution),
        "TOTAL": round(float(livraison.total_reduit_euro), 2) ,
        "PROTECTION_NAME" : protection_name,
        "DESCRIPTION_PROTECTION": protection_dercription,
...
def contract_download_(request):
    ...
    context = {
        ...
        "DATE_PERMIS_2EME":f" - Permis de conduire délivré le : {permi_desc}",
        "DATE_DEPART": date_debut,
        "HEURE_DEPART": livraison.reservation.heure_depart_char,
        "DATE_RETOUR": date_fin,
        "HEUERE_RETOUR": livraison.reservation.heure_retour_char,
        "DUREE_RESERVATION": livraison.duree_dereservation,
        "NOM_VEHICULE": livraison.modele.name,
        "MATRICULE": livraison.vehicule.matricule,
        "CAUTION": round(float(livraison.reservation.opt_protection_caution * taux), 2),
        "TOTAL": round(float(livraison.reservation.total_reduit_euro * taux), 2),
        "SIGNATURE_URL": signature_url,
        "PROTECTION_NAME" : protection_name,
        "DESCRIPTION_PROTECTION": protection_dercription,
    ...
    try:
        ...
        try:
            ...
            if to_pay_value is not None and to_pay_value > 0:
                ...
                old_total_reduit = reservation.total_reduit_euro
                ...
                html_message = render_to_string('email/achat_protection_option_email.html', {
                    ...
                    "initial_amount": old_total_reduit,
...
def verify_and_do(ref, lieu_depart, lieu_retour, date_depart, heure_depart, date_retour, heure_retour, backoffice, did_by, payment, amount__paid):
    ...
    try:
        ...
        payment_required = verify_value[0].get('payment_required') 
        refund = verify_value[0].get('refund')
        refund_amount = verify_value[0].get('refund_amount')
        current_total_days = verify_value[0].get('current_total_days')
        current_klm_limit = verify_value[0].get('current_klm_limit')
        new_total_days = verify_value[0].get('new_total_days')
        new_klm_limit = verify_value[0].get('new_klm_limit')
        diff_days = verify_value[0].get('diff_days')
        diff_klm_limit = verify_value[0].get('diff_klm_limit')
        frais = verify_value[0].get('frais')
        amount_paid = verify_value[0].get('amount_paid')
        remaining_to_pay = verify_value[0].get('remaining_to_pay')
        ...
        if verify_value and verify_value[0].get('is_available') == "yes":
            if backoffice == "yes" or (payment != "yes" and payment_required != "yes"):
                if amount_paid > 0 and diff_days > 0:
                ...
                if (reservation_obj.date_heure_debut != datetime.combine(date_depart_obj, heure_depart_obj)) or (reservation_obj.date_heure_fin != datetime.combine(date_retour_obj, heure_retour_obj)):
                    ...
                    if nouvelle_date_heure_fin > reservation_obj.date_heure_fin:
                        ...
                        reservation_obj.total_afficher += diff_prix if diff_prix > 0 else 0
                ...
                fake_request = request_factory.post(
                    ...
                    data=json.dumps({
                        ...
                        "description":f"Réservation du {reservation_obj.model_name} du : ( {date_depart} à {heure_depart} lieu de prise : {lieu_depart_name.name})→({date_retour} à {heure_retour} lieu de retour : {lieu_retour_name.name})",
    ...
    try:
        ...
        if vehicule_id :
            ...
            if promotions:
                ...
                if promotions.tout_modele == "oui" and promotions.tout_zone == "oui":
                    ...
                    if lieu_depart_obj.zone == promotions.zone_one or lieu_depart_obj.zone == promotions.zone_two or lieu_depart_obj.zone == promotions.zone_three :
                        promo_value = promotion_proportionnelle
                    else :
                        promo_value = 0
        ...
        if prime_code:
            ...
            if parent_client :
                ...
                client_resa = Reservation.objects.filter(client=client).first()
                if resa_client and not client_resa:
...
def stripe_webhook_reservation_(request):
    ...
    if event["type"] == "checkout.session.completed":
        ...
        if type_id == "reservation":
            ...
            sb_a = session.get("metadata", {}).get("sb_a")
            sb_b = session.get("metadata", {}).get("sb_b")
            sb_c = session.get("metadata", {}).get("sb_c")
            nd_driver = session.get("metadata", {}).get("nd_driver")
            nom = session.get("metadata", {}).get("nom")
            prenom = session.get("metadata", {}).get("prenom")
            birthday = session.get("metadata", {}).get("birthday")
            permis_date = session.get("metadata", {}).get("permis_date")
            ...
            if reservation.add_options == "yes":
                ...
                payment = Payment.objects.create(
                    ...
                    zone=reservation.lieu_depart.zone,  
                    total_reduit_euro=float(reservation.total_reduit_euro),
                    montant=Decimal(unit_amount),
                    montant_dzd=0,
                    montant_eur_dzd=float(unit_amount) * float(taux_change),
                    montant_dzd_eur=0,  
                    note="Complement effectué via Stripe",  
                    total_reduit_dinar=float(reservation.total_reduit_euro) * float(taux_change),
                    ecart_eur=float(reservation.reste_payer) - float(unit_amount),
                    ecart_da=(float(reservation.reste_payer) - float(unit_amount)) * float(taux_change),
                    mode_paiement="carte", 
                    total_encaisse = float(reservation.montant_paye) + float(unit_amount),  
                )
                payment.save()
            ...
            if reservation.is_complement == True:
                ...
                reservation.montant_paye += Decimal(montant)
    ...
    try:
        ...
        try:
            ...
            html_message = render_to_string('email/caution_remboursee_email.html', {
                ...
                'url' : f"{settings.API_BASE_URL}/caution-receipt-download/?livraison_id={restitution_id}"
            })
